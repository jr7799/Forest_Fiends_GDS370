...
namespace ChristinaCreatesGames.EyeMovement
{
    public class PupilTracking : MonoBehaviour
    {
        ...
        [SerializeField, Space] private string tagOfObjectToTrack = "Player";
        ...
        public void AddToEyes(Eye eye)
        {
            eyes.Add(eye);
        }
        public void RemoveFromEyes(Eye eye)
        {
            eyes.Remove(eye);
        }
        private void Awake()
        ...
        private void Update()
        {
            _targetToTrack = OBJToTrack.transform;
            _targetInVision = true;
            _currentlyWatchingForSeconds = 0;
        ...
        private IEnumerator IdleMovement(Eye eye)
        {
            while (_state == State.Idle)
            {
                ...
                yield return new WaitForSeconds(eye.DurationOfEyeResetMovement + Random.Range(-eye.DurationOfEyeResetMovement * 0.25f, 1f));
            }
        }
        ...
        private void PositionPupil(Eye eye)
        {
            ...
            Vector3 targetPupilPosition = eye.Eyeball.localPosition + Vector3.ClampMagnitude(directionToLookAt, eye.PupilMovementRadius);
            eye.Pupil.localPosition = Vector3.Lerp(eye.Pupil.localPosition, targetPupilPosition, eyeMovementSpeed * Time.deltaTime);
        }
